...
async function exampleUsage() {
  try {
    ...
    const annotationId1 = await insertAnnotation({
      place_id: placeId,
      user_id: userId,
    ...
    const annotationId2 = await insertAnnotation({
      place_id: placeId,
      user_id: userId,
    ...
    const annotationId3 = await insertAnnotation({
      place_id: placeId,
      user_id: userId,
    ...
    const similarAnnotations = await searchAnnotationsBySimilarity(mockEmbedding, 5, 0.5);
    ...
    try {
      await insertAnnotation({
        ...
        visibility: 'private' as any, // Invalid: should be 'friends' or 'public'
        rating: 3
      });
    } catch (error) {
      console.log('Validation error caught:', (error as Error).message);
