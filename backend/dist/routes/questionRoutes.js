...
router.post('/', async (req, res) => {
    try {
        ...
        const q = await db_1.default.query(`INSERT INTO questions (user_id, text, visibility, labels, metadata)
       VALUES ($1,$2,$3,$4,$5) RETURNING id, created_at`, [userId, text.trim(), visibility, Array.isArray(labels) ? labels : null, metadata || {}]);
        const id = q.rows[0].id;
        try {
            await embeddingQueue_1.embeddingQueue.enqueue('question', id, { text: text.trim(), labels, metadata }, 'normal');
        }
        catch (e) {
...
router.get('/', async (req, res) => {
    try {
        ...
        const cursorId = req.query.cursorId ? parseInt(req.query.cursorId, 10) : undefined;
        const params = [userId];
        let cursorClause = '';
...
router.post('/:questionId/answers', async (req, res) => {
    try {
        ...
        let recId = recommendation_id;
        if (!recId && recommendation_payload) {
            const payload = {
                ...recommendation_payload,
                user_id: userId,
                question_id: questionId, // Link the recommendation to the question
                metadata: { ...((recommendation_payload && recommendation_payload.metadata) || {}), questionContext: { question_id: questionId } },
            ...
            if (payload.content_type === 'place' && !payload.place_id) {
                try {
                    ...
                    const placeId = await (0, places_1.upsertPlace)(placeData);
            ...
            if (payload.content_type === 'service' && !payload.service_id) {
                try {
                    ...
                    const serviceData = {
                        ...
                        service_type: payload.content_data?.specialities ? (0, nameSimilarity_1.extractServiceType)(payload.content_data.specialities, '') || undefined : undefined,
                    ...
                    const serviceResult = await (0, serviceDeduplication_1.upsertService)(serviceData);
            ...
            recId = await (0, recommendations_1.insertRecommendation)(payload);
        }
        else if (recId) {
            // If recommendation already exists, update it to link to the question
            await db_1.default.query(`UPDATE recommendations SET question_id = $1 WHERE id = $2 AND user_id = $3`, [questionId, recId, userId]);
        }
        if (!recId) {
        ...
        const recResult = await db_1.default.query(`SELECT id, created_at FROM recommendations WHERE id = $1`, [recId]);
...
router.get('/:questionId/answers', async (req, res) => {
    try {
       ...
       LIMIT $3`, [questionId, userId, limit]);
