...
router.get('/post/:id', async (req, res) => {
    try {
        ...
        const q = await db_1.default.query(`SELECT r.id as recommendation_id, r.title, r.description, r.rating, r.created_at,
...
router.get('/question/:id', async (req, res) => {
    try {
        ...
        if (!Number.isFinite(id))
            return res.status(400).send('Invalid id');
        const q = await db_1.default.query(`SELECT q.id, q.text, u.display_name as author_name
         FROM questions q
