...
router.post('/follow/:userId', async (req, res) => {
    try {
        ...
        const success = await (0, social_1.followUser)(followerId, followingId);
        res.json({
            success: true,
...
router.delete('/unfollow/:userId', async (req, res) => {
    try {
        const followerId = req.user.id;
        const followingId = req.params.userId;
        const success = await (0, social_1.unfollowUser)(followerId, followingId);
        res.json({
            success: true,
...
router.get('/followers/:userId', async (req, res) => {
    try {
        ...
        const followers = await (0, social_1.getFollowers)(userId, limit, offset);
        res.json({
            success: true,
            data: followers,
            pagination: {
...
router.get('/following/:userId', async (req, res) => {
    try {
        ...
        const limit = parseInt(req.query.limit) || 50;
        const offset = parseInt(req.query.offset) || 0;
        const following = await (0, social_1.getFollowing)(userId, limit, offset);
        res.json({
            success: true,
            data: following,
            pagination: {
...
router.get('/users/search', async (req, res) => {
    try {
        ...
        const users = await (0, social_1.searchUsers)(query.trim(), currentUserId, limit, offset);
        res.json({
            success: true,
            data: users,
            pagination: {
...
router.get('/users/suggestions', async (req, res) => {
    try {
        ...
        const suggestions = await (0, social_1.getSuggestedUsers)(currentUserId, limit);
        res.json({
            success: true,
            data: suggestions
        });
...
router.put('/privacy-settings', async (req, res) => {
    try {
        ...
        const updatedSettings = await (0, social_1.createPrivacySettings)(userId, settings);
        res.json({
            success: true,
...
router.post('/block/:userId', async (req, res) => {
    try {
        ...
        const success = await (0, social_1.blockUser)(blockerId, blockedId);
        res.json({
            success: true,
...
router.delete('/unblock/:userId', async (req, res) => {
    try {
        const blockerId = req.user.id;
        const blockedId = req.params.userId;
        const success = await (0, social_1.unblockUser)(blockerId, blockedId);
        res.json({
            success: true,
...
router.post('/comments/:recommendationId', async (req, res) => {
    try {
        ...
        const newComment = await (0, social_1.addComment)(recommendationId, userId, comment.trim(), parent_comment_id);
        // Save mentions in the comment, if any
        try {
            const mentioned = (0, mentions_1.extractMentionUserIds)(comment);
            if (mentioned.length > 0) {
                await (0, mentions_1.saveCommentMentions)(newComment.id, mentioned, userId, comment.trim());
            }
        }
        catch (e) {
...
router.get('/comments/:recommendationId', async (req, res) => {
    try {
        ...
        const limit = parseInt(req.query.limit) || 50;
        const offset = parseInt(req.query.offset) || 0;
        const comments = await (0, social_1.getComments)(recommendationId, currentUserId, limit, offset);
        res.json({
            success: true,
            data: comments,
            pagination: {
...
router.put('/comments/:commentId', async (req, res) => {
    try {
        ...
        const success = await (0, social_1.updateComment)(commentId, userId, comment.trim());
        res.json({
            success: true,
...
router.delete('/comments/:commentId', async (req, res) => {
    try {
        const userId = req.user.id;
        const commentId = parseInt(req.params.commentId);
        const success = await (0, social_1.deleteComment)(commentId, userId);
        res.json({
            success: true,
...
router.post('/likes/recommendation/:recommendationId', async (req, res) => {
    try {
        const userId = req.user.id;
        const recommendationId = parseInt(req.params.recommendationId);
        const success = await (0, social_1.likeAnnotation)(recommendationId, userId);
        res.json({
            success: true,
            data: { liked: success },
...
router.delete('/likes/recommendation/:recommendationId', async (req, res) => {
    try {
        const userId = req.user.id;
        const recommendationId = parseInt(req.params.recommendationId);
        const success = await (0, social_1.unlikeAnnotation)(recommendationId, userId);
        res.json({
            success: true,
...
router.get('/likes/recommendation/:recommendationId', async (req, res) => {
    try {
        ...
        const [likesCount, isLiked] = await Promise.all([
            (0, social_1.getAnnotationLikesCount)(recommendationId),
            (0, social_1.isAnnotationLiked)(recommendationId, userId)
        ]);
        res.json({
            success: true,
            data: {
...
router.post('/likes/comment/:commentId', async (req, res) => {
    try {
        const userId = req.user.id;
        const commentId = parseInt(req.params.commentId);
        const success = await (0, social_1.likeComment)(commentId, userId);
        res.json({
            success: true,
            data: { liked: success },
...
router.delete('/likes/comment/:commentId', async (req, res) => {
    try {
        const userId = req.user.id;
        const commentId = parseInt(req.params.commentId);
        const success = await (0, social_1.unlikeComment)(commentId, userId);
        res.json({
            success: true,
            data: { unliked: success },
...
router.post('/saved/:placeId', async (req, res) => {
    try {
        ...
        const success = await (0, social_1.savePlace)(userId, placeId, notes);
        res.json({
            success: true,
            data: { saved: success },
...
router.delete('/saved/:placeId', async (req, res) => {
    try {
        const userId = req.user.id;
        const placeId = parseInt(req.params.placeId);
        const success = await (0, social_1.unsavePlace)(userId, placeId);
        res.json({
            success: true,
...
router.get('/saved/:placeId', async (req, res) => {
    try {
        const userId = req.user.id;
        const placeId = parseInt(req.params.placeId);
        const isSaved = await (0, social_1.isPlaceSaved)(userId, placeId);
        res.json({
            success: true,
            data: {
                is_saved: isSaved
